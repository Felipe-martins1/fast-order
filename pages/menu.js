  ...
  const addCart = (product) => {
    ...
    if (status === null) {
      ...
      if (productAddedIndex !== -1) {
        itens[productAddedIndex].qtd =
          itens[productAddedIndex].qtd + product.qtd;
        localStorage.setItem("products", JSON.stringify(itens));
      } else {
        itens.push(product);
        localStorage.setItem("products", JSON.stringify(itens));
      ...
      <section>
        <div className="pratos">
          {listItems.length > 0 &&
            listItems.map((each) => (
              <div className="prato">
                <div>
                  <img className="image" src={each.foto} alt="blog" />
                  <div className="details">
                    <h1 className="name">{each.item}</h1>
                    <h2 className="preco">
                      R${each.precoFinal}
        ...
        {listItems.length > 0 && (
          <div id="cart">
            ...
            {showCart === true && (
              <div>
                <button className="downArrow" onClick={showCartItens}>
            ...
            {showCart === false && (
              <div>
                <button className="upArrow" onClick={showCartItens}>
                  <img src="/arrow.png"></img>
                </button>
